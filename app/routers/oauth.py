"""GitHub OAuth proxy for the Decap CMS content editor at /content-admin
(not this same app's own /admin booking dashboard — two different tools).
Decap's "github" backend needs a small server that can hold the OAuth app's
client secret; this fills that role so the CMS can run without depending on
Netlify Identity.
...
environment. Point content-admin/config.yml's backend.base_url at this

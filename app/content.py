...
def get_team_photos() -> list[dict]:
    """Every photo in the "Who We Are" (Home) / "Our Story" (About) photo
    carousel, ordered for display — see app/models.py:TeamPhoto for why
    this is a separate, explicitly-curated admin section rather than
    something pulled automatically from the Gallery. Both pages fall back
    to the original static placeholder note when this comes back empty."""
    with SessionLocal() as db:
        rows = db.query(TeamPhoto).order_by(TeamPhoto.order).all()
        return [{"id": p.id, "image": p.image, "caption": p.caption} for p in rows]
